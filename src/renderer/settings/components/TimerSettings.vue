...
function formatMinutes(seconds: number): string { return `${Math.floor(seconds / 60)} 分钟` }
    ...
    <section>
      ...
      <div class="bg-white rounded-card p-4 space-y-4">
        ...
        <div>
          ...
          <input type="range" min="60" max="900" step="60" :value="config.timer.restDuration"
            class="w-full accent-primary-500"
            @input="updateTimer('restDuration', Number(($event.target as HTMLInputElement).value))" />
          <div class="flex justify-between text-xs text-gray-400 mt-1"><span>1分</span><span>15分</span></div>
        </div>
      </div>
    </section>
        ...
        <div>
          ...
          <div class="flex gap-2">
            <button v-for="st in soundTypes" :key="st.key"
              class="flex-1 py-2 text-sm rounded-btn transition-all"
              :class="config.reminder.soundType === st.key ? 'bg-primary-500 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'"
              @click="updateReminder('soundType', st.key)">{{ st.label }}</button>
          </div>
        </div>
        <div>
          <div class="flex justify-between text-sm mb-2">
